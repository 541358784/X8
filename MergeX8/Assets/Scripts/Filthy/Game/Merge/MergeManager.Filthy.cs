...
{
    private const int F_BOARD_WIDTH = 4;
    private const int F_BOARD_HEIGHT = 5;
    
    public void InitFilthyMergeBoard(MergeBoardEnum boardId)
    {
        ...
        if (!storageMergeBoardDict.ContainsKey(mergeBoardId) || GetStorageBoard(boardId).Items.Count == 0)
        {
            ...
            storageMergeBoard.Width = F_BOARD_WIDTH;
            storageMergeBoard.Height = F_BOARD_HEIGHT;
