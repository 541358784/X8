...
public partial class CardCollectionModel
{
    ...
    public bool IsResReady(CardCollectionCardThemeState state)
    {
        var assets = state.GetNeedDownloadAssets();
    ...
    public async void TryDownloadRes(CardCollectionCardThemeState state,Action<float, string> onProgressUpdate = null,Action<bool> onFinish = null)
    {
        var assets = state.GetNeedDownloadAssets();
        if (assets == null || assets.Count == 0)
        {
        ...
        while (!state.IsResReady())
        {
            UpdateProgressFromDownloadInfoList(downloadTaskList, (progress,extralInfo)=>
            {
                Debug.LogError("卡册主题"+state.CardThemeConfig.Id+" 下载进度:"+progress*100+"% "+extralInfo);
