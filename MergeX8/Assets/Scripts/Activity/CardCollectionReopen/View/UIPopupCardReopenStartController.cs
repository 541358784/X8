...
public class UIPopupCardReopenStartController : UIWindowController
{
    ...
    private StorageCardCollectionReopenActivity CurStorage;
    protected override void OnOpenWindow(params object[] objs)
    {
        base.OnOpenWindow(objs);
        CurStorage = objs[0] as StorageCardCollectionReopenActivity;
    ...
    public static UIPopupCardReopenStartController Open(StorageCardCollectionReopenActivity storage)
    {
        var theme = CardCollectionModel.Instance.GetCardThemeState(storage.ThemeId);
        var uiPath = theme.GetCardUIName(CardUIName.UIType.UIPopupCardReopenStart);
        if (!UIManager.Instance.GetOpenedUIByPath(uiPath))
            return UIManager.Instance.OpenUI(uiPath,storage) as UIPopupCardReopenStartController;
        return UIManager.Instance.GetOpenedUIByPath(uiPath) as UIPopupCardReopenStartController;
    }
}