...
public partial class UITurtlePangShopController:UIWindowController
{
    ...
    public override void PrivateAwake()
    {
        ...
        {
            // GuideSubSystem.Instance.FinishCurrent(GuideTargetType.TurtlePangPreview);
            var mainUI = UIManager.Instance.GetOpenedUIByPath<UITurtlePangMainController>(UINameConst.UITurtlePangMain);
            if (mainUI)
                mainUI.AnimCloseWindow();
            AnimCloseWindow(() =>
            {
                var rewards = new List<int>();
                foreach (var pair in TurtlePangModel.Instance.StoreItemConfig)
                {
                    var config = pair.Value;
                    var type = (TurtlePangUtils.TurtlePangStoreItemType)config.Type;
                    if (type == TurtlePangUtils.TurtlePangStoreItemType.BuildItem)
                    {
                ...
                // {
                //     if (rewards[i] == TurtlePangModel.Instance.GlobalConfig.CenterDecoItem)
                //     {
                //         rewards.RemoveAt(i);
                //         rewards.Insert(0,TurtlePangModel.Instance.GlobalConfig.CenterDecoItem);
                //         break;
                //     }
                // }
                Action callback = ()=>
    ...
    protected override void OnOpenWindow(params object[] objs)
    {
        ...
        UpdateScoreText();
        var storeLevelConfigList = TurtlePangModel.Instance.StoreLevelConfig;
    ...
    private float UnitBaseHeight = 85;
    private float UnitRowHeight = 260;
    private float RankGroupHeight = 0;
    ...
    public static UITurtlePangShopController Instance;
    public static UITurtlePangShopController Open(StorageTurtlePang storageTurtlePang)
    {
        Instance = UIManager.Instance.OpenUI(UINameConst.UITurtlePangShop, storageTurtlePang) as
            UITurtlePangShopController;
        return Instance;
    }
}