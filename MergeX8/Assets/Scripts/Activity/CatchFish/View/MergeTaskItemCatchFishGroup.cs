...
public class MergeTaskItemCatchFishGroup : MonoBehaviour
{
    ...
    public void Refresh()
    {
        gameObject.SetActive(CatchFishModel.Instance.IsStart() && !MainOrderManager.Instance.IsSpecialTask(StorageTask.OrgId));
        if (gameObject.activeSelf)
        {
            NumText.SetText(CatchFishModel.Instance.GetTaskValue(StorageTask).ToString());
    ...
    public void CollectReward( ref List<ResData> resDatas, bool autoAdd = true)
    {
        ...
        var num = UserData.Instance.GetRes(UserData.ResourceId.CatchFish);
        var multiValue = 1f;
        var count = CatchFishModel.Instance.GetTaskValue(StorageTask);
        count = (int)(count*multiValue);
        if (autoAdd && count > 0)
        {
            UserData.Instance.AddRes((int) UserData.ResourceId.CatchFish, count,
            ...
            resDatas.Add(new ResData(UserData.ResourceId.CatchFish, count));
        }
        var storage = CatchFishModel.Instance.Storage;
        var entrance = DynamicEntryManager.Instance.GetDynamicEntryMonoBehaviour<MergeCatchFish, DynamicEntry_Game_CatchFish>();
        if (!entrance)
            return;
        entrance.LockNum();
