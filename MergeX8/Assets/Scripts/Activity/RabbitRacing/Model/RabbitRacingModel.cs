...
using UnityEngine;
using DragonPlus;
using DragonPlus.Config.RabbitRacing;
...
{
    public class RabbitRacingModel : ActivityEntityBase
    {
        public override string Guid => "OPS_EVENT_TYPE_RABBIT_RACING";

        private static RabbitRacingModel _instance;
        public static RabbitRacingModel Instance => _instance ?? (_instance = new RabbitRacingModel());
        ...
        {
            get { return StorageManager.Instance.GetStorage<StorageHome>().RabbitRacing; }
        }
        ...
        /// 正常配置
        /// </summary>
        private List<TableRabbitRacingReward> _listNormalConfigs = new List<TableRabbitRacingReward>();

        /// <summary>
        /// 循环配置
        /// </summary>
        private List<TableRabbitRacingReward> _listCycleConfigs = new List<TableRabbitRacingReward>();


        /// <summary>
        /// 当前阶段
        /// </summary>
        public TableRabbitRacingReward CurRacing;

        private string coolTimeKey = "RabbitRacing";
            ...
            ulong rewardEndTime, bool manualEnd, string configJson, string activitySubType)
        {
            ...
            RabbitRacingConfigManager.Instance.InitConfig(configJson);
        ...
        {
            return UnlockManager.IsUnlockSoon(UnlockManager.MergeUnlockType.RabbitRacing);
        }
        ...
        private void InitConfig()
        {
            ...
            _listNormalConfigs = RabbitRacingConfigManager.Instance.TableRabbitRacingRewardList.FindAll(p => p.Stage == 1);
            _listCycleConfigs = RabbitRacingConfigManager.Instance.TableRabbitRacingRewardList.FindAll(p => p.Stage == 2);
        ...
        public void JoinOrInitRacing(bool isJoin)
        {
            ...
            if (Storage.CurRacingIndex == 0)
            {
                if (isJoin)
                {
                    ...
                    Storage.RunRounds = 1;
                    CurRacing = RabbitRacingConfigManager.Instance.TableRabbitRacingRewardList.Find(p =>
                        p.Id == Storage.CurRacingIndex);
                    InitRacingPlayer();
                    Storage.IsDone = false;
                    Storage.IsAward = false;
                    Storage.DoneList.Clear();
                    EventDispatcher.Instance.DispatchEvent(EventEnum.RABBIT_RACING_JOIN);
                    GameBIManager.Instance.SendGameEvent(BiEventAdventureIslandMerge.Types.GameEventType.GameEventRabbitRaceStart,
            ...
            else
            {
                if (Storage.IsDone)
                {
                    if (isJoin)
                    {
                        ...
                        {
                            TableRabbitRacingReward last =
                                RabbitRacingConfigManager.Instance.TableRabbitRacingRewardList.Find(p =>
                                    p.Id == Storage.CurRacingIndex);
        ...
        private void InitRacingPlayer()
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                {
                    loopNum++;
                    name = RabbitRacingConfigManager.Instance.TableRabbitRacingRobotNameList.RandomPickOne().Name;
        ...
        {
            TableRabbitRacingRobot robotConfig =
                RabbitRacingConfigManager.Instance.TableRabbitRacingRobotList.Find(p => p.Id == CurRacing.Id);
            ...
            {
                TableRabbitRacingScore score = RabbitRacingConfigManager.Instance.TableRabbitRacingScoreList.First();

                //第一轮变动默认第一组配置
                if (firstCal)
                {
                    score = RabbitRacingConfigManager.Instance.TableRabbitRacingScoreList.Find(p => p.Id == scoreGroup[0]);
                    firstCal = false;
                }
                else
                {
                    int group = scoreGroup[scoreWeight.RandomIndexByWeight()];
                    score = RabbitRacingConfigManager.Instance.TableRabbitRacingScoreList.Find(p => p.Id == group);
                }
        ...
        private void CalRacingEnd()
        {
            ...
            for (int i = 0; i < Storage.PlayerList.Count; i++)
            {
                ...
                if (cur.CurScore >= CurRacing.Collect && !Storage.DoneList.Contains(cur.Seat))
                {
                    ...
                    {
                        Storage.IsDone = true;
                        EventDispatcher.Instance.DispatchEvent(EventEnum.RABBIT_RACING_DONE);
        ...
        public void CheckRobotScoreUpdate()
        {
            ...
            {
                SortRank();
                EventDispatcher.Instance.DispatchEvent(EventEnum.RABBIT_RACING_SCORE_UPDATE, listChange);
            }
        }
        ...
        /// <returns></returns>
        private TableRabbitRacingReward GetNextConfig(TableRabbitRacingReward config)
        {
        ...
        public void AddMergeScore(int mergeId, Vector3 scrPos, string from)
        {
            ...
            TableRabbitRacingItem item = RabbitRacingConfigManager.Instance.TableRabbitRacingItemList.Find(p => p.Id == mergeId);
        ...
        public void AddBubbleScore(int diamond, Vector3 scrPos)
        {
            ...
            int Coefficient = RabbitRacingConfigManager.Instance.TableRabbitRacingSettingList[0].Bubble_coefficient;
        ...
        public void AddFlashBuyScore(int diamond, Vector3 scrPos)
        {
            ...
            int Coefficient = RabbitRacingConfigManager.Instance.TableRabbitRacingSettingList[0].FlashBuy_coefficient;
        ...
        public void AddRandomTaskScore(int seat, int coin, Vector3 scrPos)
        {
            ...
            int Coefficient = RabbitRacingConfigManager.Instance.TableRabbitRacingSettingList[0].Order_coefficient;
        ...
        public void FlyScore(int count, Vector3 scrPos)
        {
            ...
            var gameEntry = DynamicEntryManager.Instance.GetDynamicEntryMonoBehaviour<Game_RabbitRacing>(typeof(DynamicEntry_Game_RabbitRacing));
            ...
            {
                var homeEntry = DynamicEntryManager.Instance.GetDynamicEntryMonoBehaviour<Home_RabbitRacing>(typeof(DynamicEntry_Home_RabbitRacing));
                if (homeEntry != null)
            ...
            while (count > 0)
            {
                ...
                FlyGameObjectManager.Instance.FlyObject(gameEntry.Target, scrPos, endPos, true, 0.7f, 0.1f * index,
                    () =>
                    {
                        ...
                        EventDispatcher.Instance.DispatchEventImmediately(EventEnum.RABBIT_STAGE_COLLECTION_FLY_SUCCESS, change);
                        if (isEnd)
        ...
        {
            return base.IsOpened() && UnlockManager.IsOpen(UnlockManager.MergeUnlockType.RabbitRacing);
        }
        ...
        public bool CanShowJoinRacing()
        {
            ...
            if (Storage.CurRacingIndex <= 0 || (Storage.CurRacingIndex > 0 && Storage.IsAward))
            {
                if (!Storage.IsJoin)
                {
                    UIManager.Instance.OpenUI(UINameConst.UIPopupRabbitRacingStart);
                }
                else
                {
        ...
        public bool TimeCheckRacingEnding()
        {
            ...
            if (UIManager.Instance.GetOpenedUIByPath(UINameConst.UIRabbitRacingMain) != null)
                return false;
            ...
            if (Storage.IsDone)
            {
                UIManager.Instance.OpenUI(UINameConst.UIRabbitRacingMain, false);
                return true;
            }
        ...
        public void TryOpenMain()
        {
            ...
            {
                UIManager.Instance.OpenUI(UINameConst.UIRabbitRacingMain);
            }
        }
    }
}