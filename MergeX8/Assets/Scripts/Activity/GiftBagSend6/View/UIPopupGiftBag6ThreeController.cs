...
public class UIPopupGiftBagSend6Controller:UIWindowController
{
    private static string coolTimeKey = "UIPopupGiftBagSend6";
    public static bool CanShowUI()
    {
        if (!GiftBagSend6Model.Instance.IsOpened())
            return false;
    ...
    public static UIPopupGiftBagSend6Controller Instance;
    public static UIPopupGiftBagSend6Controller Open()
    {
        if (Instance)
            Instance.CloseWindowWithinUIMgr(true);
        Instance =
            UIManager.Instance.OpenUI(UINameConst.UIPopupGiftBagSend6) as UIPopupGiftBagSend6Controller;
        return Instance;
    }
    ...
    private StorageGiftBagSend6 Storage => GiftBagSend6Model.Instance.StorageGiftBagSend6;
    ...
    protected override void OnOpenWindow(params object[] objs)
    {
        ...
        RewardConfig = GiftBagSend6Model.Instance.GetGiftBagSend6Resources();
        var i = 0;
    ...
    {
        TimeText.SetText(GiftBagSend6Model.Instance.GetActivityLeftTimeString());
        if (GiftBagSend6Model.Instance.GetActivityLeftTime() <= 0)
        {
            AnimCloseWindow();
    ...
    {
        private GiftBagSend6Resource Config;
        ...
        private StorageGiftBagSend6 Storage => GiftBagSend6Model.Instance.StorageGiftBagSend6;
        private UIPopupGiftBagSend6Controller Controller;
        public void Init(GiftBagSend6Resource config,UIPopupGiftBagSend6Controller controller)
        {
            Config = config;
            ...
            if (_rvRebornButton)
            {
                ...
                rvBtn = UIAdRewardButton.Create(ADConstDefine.R_GIFT_BAG_SEND_6, UIAdRewardButton.ButtonStyle.Disable, _rvRebornButton.gameObject,
                    (s) =>
                    {
                        if (s)
                        {
                            DestroyImmediate(rvBtn);
                            GiftBagSend6Model.Instance.OnRvSuccess(Config);
                        }
                    }, false, null, () =>
                    {
                    });
            ...
            if (CollectBtn)
            {
                ...
                {
                    GiftBagSend6Model.Instance.GiftBagSend6GetReward(Config);
                    Controller.OnBuy();
                    // UpdateState();
                });
            }
        ...
        public void UpdateState()
        {
            ...
            else if (GiftBagSend6Model.Instance.CanCollect(Config))
            {
