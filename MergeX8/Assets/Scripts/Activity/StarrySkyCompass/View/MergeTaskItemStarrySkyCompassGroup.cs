...
public class MergeTaskItemStarrySkyCompassGroup : MonoBehaviour
{
    ...
    public void Refresh()
    {
        gameObject.SetActive(StarrySkyCompassModel.Instance.IsStart() && !MainOrderManager.Instance.IsSpecialTask(StorageTask.OrgId));
        if (gameObject.activeSelf)
        {
            NumText.SetText(StarrySkyCompassModel.Instance.GetTaskValue(StorageTask, false).ToString());
    ...
    public void CollectReward(ref List<ResData> resDatas, bool autoAdd = true)
    {
        ...
        var num = UserData.Instance.GetRes(UserData.ResourceId.StarrySkyCompassRocket);
        var multiValue = 1f;
        var count = StarrySkyCompassModel.Instance.GetTaskValue(StorageTask, multiValue == 1);
        count = (int)(count*multiValue);
        if (autoAdd && count > 0)
        {
            UserData.Instance.AddRes((int) UserData.ResourceId.StarrySkyCompassRocket, count,
            ...
            resDatas.Add(new ResData(UserData.ResourceId.StarrySkyCompassRocket, count));
        }
        var storage = StarrySkyCompassModel.Instance.Storage;
        var entrance = DynamicEntryManager.Instance.GetDynamicEntryMonoBehaviour<MergeStarrySkyCompass, DynamicEntry_Game_StarrySkyCompass>();
        if (!entrance)
            return;
        entrance.LockNum();
