...
public partial class UIFlowerFieldMainController
    {
        public void CheckLeaderBaordGuide()
        {
            ...
            if (leaderBoardStorage != null && leaderBoardStorage.IsInitFromServer() && !GuideSubSystem.Instance.isFinished(GuideTriggerPosition.FlowerFieldLeaderBoardMainEntrance))
            {
                List<Transform> topLayer = new List<Transform>();
                topLayer.Add(LeaderBoard.transform);
                GuideSubSystem.Instance.RegisterTarget(GuideTargetType.FlowerFieldLeaderBoardMainEntrance, LeaderBoard.transform as RectTransform,
                    topLayer: topLayer);
                if (GuideSubSystem.Instance.Trigger(GuideTriggerPosition.FlowerFieldLeaderBoardMainEntrance, null))
                {
        ...
        public void AwakeRank()
        {
            ...
            LeaderBoard.Init(FlowerFieldModel.Instance.Storage, this);
        }
        ...
        public class LeaderBoardEntrance : MonoBehaviour
        {
            ...
            private StorageFlowerField Storage;
            private StorageCommonLeaderBoard LeaderBoardStorage;
            private UIFlowerFieldMainController MainUI;

            public void Init(StorageFlowerField storage, UIFlowerFieldMainController mainUI)
            {
                Storage = storage;
                // if (Storage.IsEnd)
                // {
                ...
                LeaderBoardStorage = FlowerFieldLeaderBoardModel.Instance.GetLeaderBoardStorage(Storage.ActivityId);
                ...
                // TipText.SetTerm("ui_FlowerFieldTraining_rank_req");
                ...
                EventDispatcher.Instance.AddEvent<EventFlowerFieldScoreChange>(OnScoreChange);
            }
            ...
            private void OnDestroy()
            {
                ...
                EventDispatcher.Instance.RemoveEvent<EventFlowerFieldScoreChange>(OnScoreChange);
            }
            // private bool lastState = false;
            // public void UpdateUI()
            // {
            //     // TipText.SetText( LocalizationManager.Instance.GetLocalizedStringWithFormat("UI_EasterEggActive_Shop_Ranking_Desc",FlowerFieldLeaderBoardModel.Instance.LeastEnterBoardScore.ToString()));
            //     TipText.SetTermFormats(LeaderBoardStorage.LeastStarCount.ToString());
            ...
            public void RefreshView()
            {
                ...
                // if (!GuideSubSystem.Instance.isFinished(GuideTriggerPosition.FlowerFieldLeaderBoardEntrance) && LeaderBoardStorage.IsStorageWeekInitFromServer())
                // {
                //     Action<Action> performAction = (callback) =>
                //     {
                //         List<Transform> topLayer = new List<Transform>();
                //         topLayer.Add(RankBtn.transform);
                //         GuideSubSystem.Instance.RegisterTarget(GuideTargetType.FlowerFieldLeaderBoardEntrance, RankBtn.transform as RectTransform,
                //             topLayer: topLayer);
                //         if (GuideSubSystem.Instance.Trigger(GuideTriggerPosition.FlowerFieldLeaderBoardEntrance, null))
                //         {
                //             GuideTriggerPosition.FlowerFieldLeaderBoardEntrance.WaitGuideFinish()
            ...
            {
                GuideSubSystem.Instance.FinishCurrent(GuideTargetType.FlowerFieldLeaderBoardMainEntrance);
                // if (MainUI.IsPlaying())
                //     return;
                if (!LeaderBoardStorage.IsInitFromServer())
                {
                ...
                // UnlockEffect.gameObject.SetActive(false);
                FlowerFieldLeaderBoardModel.Instance.OpenMainPopup(LeaderBoardStorage);
            }
            ...
            public void OnScoreChange(EventFlowerFieldScoreChange evt)
            {
