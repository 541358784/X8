...
public class UIDogHopeLeaderBoardMainController:UIWindowController
{
    ...
    private StorageDogHopeLeaderBoard _storageWeek;
    ...
    public override void PrivateAwake()
    {
        ...
        // lockText.SetText(LocalizationManager.Instance.GetLocalizedStringWithFormat("UI_adventurecoin_race_require",DogHopeLeaderBoardModel.Instance.LeastEnterBoardScore.ToString()));
    ...
    protected override async void OnOpenWindow(params object[] objs)  
    {
        ...
        RefreshUI();
        GuideSubSystem.Instance.Trigger(GuideTriggerPosition.DogHopeLeaderBoardDescribe, null);
    }
    
    public void BindStorageWeek(StorageDogHopeLeaderBoard storageWeek)
    {
    ...
    public void RefreshUI()
    {
        ...
        _buttonClose.gameObject.SetActive(!_storageWeek.CanStorageDogHopeLeaderBoardGetReward());
        _buttonCollect.gameObject.SetActive(_storageWeek.CanStorageDogHopeLeaderBoardGetReward());
        _buttonShop.gameObject.SetActive(false);
        //更新ui排名逻辑
    ...
    async void OnCollectBtn()
    {
        ...
        if (_storageWeek.CanStorageDogHopeLeaderBoardGetReward())
        {
            var success = await _storageWeek.QuitLeaderBoardFromServer();
            if (!success)
                return;
            //没有BI暂时注释掉
            var reasonArgs = new GameBIManager.ItemChangeReasonArgs(){reason = BiEventAdventureIslandMerge.Types.ItemChangeReason.DogHopeLeaderBoardGet};
            ...
            DragonPlus.GameBIManager.Instance.SendGameEvent(
                BiEventAdventureIslandMerge.Types.GameEventType.GameEventDogHopeLeaderBoard,
    ...
    //         return;
    //     var startPopup = UIManager.Instance.OpenUI(UINameConst.UIDogHopeLeaderBoardStart,true) as UIDogHopeLeaderBoardStartController;
    // }
    ...
    public static UIDogHopeLeaderBoardMainController Open(StorageDogHopeLeaderBoard storageDogHope)
    {
        var mainWindow = UIManager.Instance.OpenUI(UINameConst.UIDogHopeLeaderBoardMain) as UIDogHopeLeaderBoardMainController;
        if (!mainWindow)
            return null;
        mainWindow.BindStorageWeek(storageDogHope);
        return mainWindow;
    }
}