...
public class UIClimbTreeLeaderBoardMainController:UIWindowController
{
    ...
    private StorageClimbTreeLeaderBoard _storageWeek;
    ...
    public override void PrivateAwake()
    {
        ...
        // lockText.SetText(LocalizationManager.Instance.GetLocalizedStringWithFormat("UI_adventurecoin_race_require",ClimbTreeLeaderBoardModel.Instance.LeastEnterBoardScore.ToString()));
    ...
    protected override async void OnOpenWindow(params object[] objs)  
    {
        ...
        RefreshUI();
        GuideSubSystem.Instance.Trigger(GuideTriggerPosition.ClimbTreeLeaderBoardDescribe, null);
    }
    
    public void BindStorageWeek(StorageClimbTreeLeaderBoard storageWeek)
    {
    ...
    public void RefreshUI()
    {
        ...
        _buttonClose.gameObject.SetActive(!_storageWeek.CanStorageClimbTreeLeaderBoardGetReward());
        _buttonCollect.gameObject.SetActive(_storageWeek.CanStorageClimbTreeLeaderBoardGetReward());
        _buttonShop.gameObject.SetActive(false);
        //更新ui排名逻辑
    ...
    async void OnCollectBtn()
    {
        ...
        if (_storageWeek.CanStorageClimbTreeLeaderBoardGetReward())
        {
            var success = await _storageWeek.QuitLeaderBoardFromServer();
            if (!success)
                return;
            //没有BI暂时注释掉
            var reasonArgs = new GameBIManager.ItemChangeReasonArgs(){reason = BiEventAdventureIslandMerge.Types.ItemChangeReason.ClimbTreeLeaderBoardGet};
            ...
            DragonPlus.GameBIManager.Instance.SendGameEvent(
                BiEventAdventureIslandMerge.Types.GameEventType.GameEventClimbTreeLeaderBoard,
    ...
    //         return;
    //     var startPopup = UIManager.Instance.OpenUI(UINameConst.UIClimbTreeLeaderBoardStart,true) as UIClimbTreeLeaderBoardStartController;
    // }
    ...
    public static UIClimbTreeLeaderBoardMainController Open(StorageClimbTreeLeaderBoard storageClimbTree)
    {
        var mainWindow = UIManager.Instance.OpenUI(UINameConst.UIClimbTreeLeaderBoardMain) as UIClimbTreeLeaderBoardMainController;
        if (!mainWindow)
            return null;
        mainWindow.BindStorageWeek(storageClimbTree);
        return mainWindow;
    }
}