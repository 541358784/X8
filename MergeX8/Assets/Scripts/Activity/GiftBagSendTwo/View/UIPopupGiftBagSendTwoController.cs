...
public class UIPopupGiftBagSendTwoController:UIWindowController
{
    private static string coolTimeKey = "UIPopupGiftBagSendTwo";
    public static bool CanShowUI()
    {
        if (!GiftBagSendTwoModel.Instance.IsOpened())
            return false;
    ...
    public static UIPopupGiftBagSendTwoController Instance;
    public static UIPopupGiftBagSendTwoController Open()
    {
        if (Instance)
            Instance.CloseWindowWithinUIMgr(true);
        Instance =
            UIManager.Instance.OpenUI(UINameConst.UIPopupGiftBagSendTwo) as UIPopupGiftBagSendTwoController;
        return Instance;
    }
    ...
    private StorageGiftBagSendTwo Storage => GiftBagSendTwoModel.Instance.StorageGiftBagSendTwo;
    ...
    protected override void OnOpenWindow(params object[] objs)
    {
        ...
        RewardConfig = GiftBagSendTwoModel.Instance.GetGiftBagSendTwoResources();
        var i = 0;
    ...
    {
        TimeText.SetText(GiftBagSendTwoModel.Instance.GetActivityLeftTimeString());
        if (GiftBagSendTwoModel.Instance.GetActivityLeftTime() <= 0)
        {
            AnimCloseWindow();
    ...
    {
        private GiftBagSendTwoResource Config;
        ...
        private StorageGiftBagSendTwo Storage => GiftBagSendTwoModel.Instance.StorageGiftBagSendTwo;
        public void Init(GiftBagSendTwoResource config)
        {
            Config = config;
            ...
            if (_rvRebornButton)
            {
                ...
                rvBtn = UIAdRewardButton.Create(ADConstDefine.R_GIFT_BAG_SEND_2, UIAdRewardButton.ButtonStyle.Disable, _rvRebornButton.gameObject,
                    (s) =>
                    {
                        if (s)
                        {
                            DestroyImmediate(rvBtn);
                            GiftBagSendTwoModel.Instance.OnRvSuccess(Config);
                        }
                    }, false, null, () =>
                    {
                    });
            ...
            if (CollectBtn)
            {
                ...
                {
                    GiftBagSendTwoModel.Instance.GiftBagSendTwoGetReward(Config);
                    UpdateState();
                });
            }
        ...
        public void UpdateState()
        {
            ...
            else if (GiftBagSendTwoModel.Instance.CanCollect(Config))
            {
