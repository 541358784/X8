...
public class UIPopupSummerWatermelonMainController:UIWindowController
{
    public SummerWatermelonMergeBoard _mergeBoard;
    ...
    private StorageSummerWatermelon StorageSummerWatermelon => SummerWatermelonModel.Instance.StorageSummerWatermelon;
    public StorageList<int> UnSetItems => SummerWatermelonModel.Instance.UnSetItems;
    public StorageList<int> UnSetRewards => SummerWatermelonModel.Instance.UnSetRewards;
    ...
    protected override void OnOpenWindow(params object[] objs)
    {
        ...
        _isGuide = !GuideSubSystem.Instance.isFinished(GuideTriggerPosition.SummerWatermelonDes);//有引导时强制显示play按钮，隐藏临时背包按钮
        MergeManager.Instance.Refresh(MergeBoardEnum.SummerWatermelon);
        _mergeBoard = transform.Find("Root").GetComponentDefault<SummerWatermelonMergeBoard>("Board");
        _mergeBoard.activeIndex = 20;
        // mergeBoard.SetBoardID((int)MergeBoardEnum.SummerWatermelon);
        RefreshView();
        if (_isGuide)
        {
            GuideSubSystem.Instance.RegisterTarget(GuideTargetType.SummerWatermelonEnterGame, _playBtn.transform as RectTransform, topLayer: new List<Transform>()
            {
                _playBtn.transform
            });
            GuideSubSystem.Instance.Trigger(GuideTriggerPosition.SummerWatermelonDes, null);
        }
        else if (!GuideSubSystem.Instance.isFinished(GuideTriggerPosition.SummerWatermelonProgressInfo))
        {
            var progressTransform = transform.Find("Root/Scroll View");
            GuideSubSystem.Instance.RegisterTarget(GuideTargetType.SummerWatermelonProgressInfo, progressTransform as RectTransform);
            GuideSubSystem.Instance.RegisterTarget(GuideTargetType.SummerWatermelonUnSetItem, _newItemBtn.transform as RectTransform, topLayer: new List<Transform>()
            {
                _newItemBtn.transform
            });
            GuideSubSystem.Instance.Trigger(GuideTriggerPosition.SummerWatermelonProgressInfo, null);
        }
    }
    ...
    {
        _progressSlider.value = Math.Max(0,StorageSummerWatermelon.MaxUnlockLevel - 1);
        foreach (var pair in _iconGroupList)
        {
            pair.Value.gameObject.SetActive(pair.Key <= StorageSummerWatermelon.MaxUnlockLevel);
        }
    }

    public Task GrowProgressView()
    {
        var curSliderValue = Math.Max(0, StorageSummerWatermelon.MaxUnlockLevel - 1);
        if ((int)_progressSlider.value != curSliderValue)
        {
            ...
            {
                RefreshProgressView();
                ShowProgressUnlockEffect(StorageSummerWatermelon.MaxUnlockLevel);
                task.SetResult(true);
    ...
    public static GameObject ProgressEffectObj
    {
        get
        {
            ...
            {
                var prefab = ResourcesManager.Instance.LoadResource<GameObject>("Prefabs/Activity/SummerWatermelonNormal/VFX_Hint_Stars_002");
    ...
    void OnNewItemBtn()
    {
        ...
        GuideSubSystem.Instance.FinishCurrent(GuideTargetType.SummerWatermelonUnSetItem);
        if ((UnSetRewards.Count + UnSetItems.Count) == 0)
            return;
        int emptyIndex = MergeManager.Instance.FindEmptyGrid(_mergeBoard.activeIndex,MergeBoardEnum.SummerWatermelon);
        if (emptyIndex == -1)
        {
        ...
        MergeManager.Instance.SetNewBoardItem(emptyIndex, storageMergeItem.Id, 1, RefreshItemSource.rewards, MergeBoardEnum.SummerWatermelon,-1, false);
        ...
        FlyGameObjectManager.Instance.FlyObject(storageMergeItem.Id, _newItemBtn.transform.position,
            ...
            () =>
            {
                ...
                EventDispatcher.Instance.DispatchEventImmediately(MergeEvent.MERGE_BOARD_REFRESH, MergeBoardEnum.SummerWatermelon,emptyIndex, -1,
    ...
    void OnPlayBtn()
    {
        ...
        GuideSubSystem.Instance.FinishCurrent(GuideTargetType.SummerWatermelonEnterGame);
        AnimCloseWindow(() =>
        {
            if (SceneFsm.mInstance.GetCurrSceneType() != StatusType.Game)
            {
                if (!GuideSubSystem.Instance.isFinished(GuideTriggerPosition.SummerWatermelonGameEntrance))
                {
                    ...
                    SummerWatermelonModel.InGuideChain = true;   
                }
