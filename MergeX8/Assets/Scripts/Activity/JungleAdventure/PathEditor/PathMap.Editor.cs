...
namespace JungleAdventure
{
    public partial class PathMap
    {
        ...
        public void ExportPath()
        {
            ...
            _segments.ForEach((a) =>
            {
                _points.Add(new SerializablePointList());
            });
            ...
            //增加第一个点
            _points[segmentIndex].points.Add(transform.InverseTransformPoint(curPosition));
            ...
            while (nextSegment != null)
            {
                ...
                _points[segmentIndex].points.Add(transform.InverseTransformPoint(nextPos));

                lenght += Vector2.Distance(curPosition, nextPos);
                
                curPosition = nextPos;

                if (_currentJurney >= 1)
                {
                    //_points[segmentIndex].points.Add(transform.InverseTransformPoint(nextSegment.p2.position));
                    _pathLength.Add(lenght);
        ...
        private void OnDrawGizmos()
        {
            ...
            if (_points != null)
            {
                ...
                {
                    for (int j = 0; j < _points[i].points.Count; j++)
                    {
                        if (j == 0 || j == _points[i].points.Count-1)
                            Gizmos.color = Color.red;
                        else
                            Gizmos.color = Color.white;
                        
                        Gizmos.DrawSphere(_points[i].points[j], 0.05f);
                    }
                }
            }
        }
