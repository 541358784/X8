...
public class OptionalGiftSelectItem : MonoBehaviour
{
    ...
    public void Init(int index ,OptionalGiftActivityConfig config)
    {
        _index = index;
        switch (index)
        {
        ...
        {
            int select= OptionGiftModel.Instance.GetSelect(index);
            if (select > 0)
            {
                InitItem(_rewardItem,select,count[item.IndexOf(select)]);
                _addButton.gameObject.SetActive(false);
    ...
    {
        UIManager.Instance.OpenUI(UINameConst.UIPopupOptionalGiftSelect,_index,item,count);
    }
