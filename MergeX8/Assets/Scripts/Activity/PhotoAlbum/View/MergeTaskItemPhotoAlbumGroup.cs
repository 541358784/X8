...
public class MergeTaskItemPhotoAlbumGroup : MonoBehaviour
{
    ...
    public void Refresh()
    {
        gameObject.SetActive(PhotoAlbumModel.Instance.IsStart() && !MainOrderManager.Instance.IsSpecialTask(StorageTask.OrgId) && !PhotoAlbumModel.Instance.IsFinish);
        if (gameObject.activeSelf)
        {
            NumText.SetText(PhotoAlbumModel.Instance.GetTaskValue(StorageTask, false).ToString());
    ...
    public void CollectReward(ref List<ResData> resDatas, bool autoAdd = true)
    {
        ...
        var num = UserData.Instance.GetRes(UserData.ResourceId.PhotoAlbumScore);
        var multiValue = 1f;
        var count = PhotoAlbumModel.Instance.GetTaskValue(StorageTask, multiValue == 1);
        count = (int)(count*multiValue);
        if (autoAdd && count > 0)
        {
            UserData.Instance.AddRes((int) UserData.ResourceId.PhotoAlbumScore, count,
            ...
            resDatas.Add(new ResData(UserData.ResourceId.PhotoAlbumScore, count));
        }
        var storage = PhotoAlbumModel.Instance.Storage;
        var entrance = DynamicEntryManager.Instance.GetDynamicEntryMonoBehaviour<MergePhotoAlbum, DynamicEntry_Game_PhotoAlbum>();
        if (!entrance)
            return;
        entrance.LockNum();
