...
public class MergeTaskItemZumaGroup : MonoBehaviour
{
    ...
    public void Refresh()
    {
        gameObject.SetActive(ZumaModel.Instance.IsStart() && !MainOrderManager.Instance.IsSpecialTask(StorageTask.OrgId));
        if (gameObject.activeSelf)
        {
            NumText.SetText(ZumaModel.Instance.GetTaskValue(StorageTask, false).ToString());
    ...
    public void CollectReward(ref List<ResData> resDatas, bool autoAdd = true)
    {
        ...
        if (!ZumaModel.Instance.IsStart())
            return;
        
        var num = UserData.Instance.GetRes(UserData.ResourceId.ZumaBall);
        var multiValue = 1f;
        var count = ZumaModel.Instance.GetTaskValue(StorageTask, multiValue == 1);
        ...
        if (autoAdd && count > 0)
        {
            ...
            resDatas.Add(new ResData(UserData.ResourceId.ZumaBall, count));
        }
        var storage = ZumaModel.Instance.CurStorageZumaWeek;
        var entrance = DynamicEntryManager.Instance.GetDynamicEntryMonoBehaviour<MergeZuma ,DynamicEntry_Game_Zuma>();
        if (!entrance)
            return;
        entrance.LockNum();
