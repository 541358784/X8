...
public partial class UIZumaMainController
{
    ...
    public class LeaderBoardEntrance : MonoBehaviour
    {
        ...
        {
            Storage = storage;
            LeaderBoardStorage = ZumaLeaderBoardModel.Instance.GetLeaderBoardStorage(Storage.ActivityId);
        ...
        // {
        //     // TipText.SetText( LocalizationManager.Instance.GetLocalizedStringWithFormat("UI_EasterEggActive_Shop_Ranking_Desc",ZumaLeaderBoardModel.Instance.LeastEnterBoardScore.ToString()));
        ...
        public void RefreshView()
        {
            ...
            // if (!GuideSubSystem.Instance.isFinished(GuideTriggerPosition.ZumaLeaderBoardEntrance) && LeaderBoardStorage.IsStorageWeekInitFromServer())
            // {
            //     Action<Action> performAction = (callback) =>
            //     {
            //         List<Transform> topLayer = new List<Transform>();
            //         topLayer.Add(RankBtn.transform);
            //         GuideSubSystem.Instance.RegisterTarget(GuideTargetType.ZumaLeaderBoardEntrance, RankBtn.transform as RectTransform,
            //             topLayer: topLayer);
            //         if (GuideSubSystem.Instance.Trigger(GuideTriggerPosition.ZumaLeaderBoardEntrance, null))
            //         {
            //             GuideTriggerPosition.ZumaLeaderBoardEntrance.WaitGuideFinish()
        ...
        {
            // GuideSubSystem.Instance.FinishCurrent(GuideTargetType.ZumaLeaderBoardEntrance);
            if (MainUI.IsPlaying())
                return;
            // if (!LeaderBoardStorage.IsInitFromServer())
            // {
            ...
            // }
            // UnlockEffect.gameObject.SetActive(false);
            ZumaLeaderBoardModel.Instance.OpenMainPopup(LeaderBoardStorage);
        }
