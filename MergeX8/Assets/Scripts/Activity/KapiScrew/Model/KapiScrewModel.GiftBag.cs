...
using UnityEngine;
    ...
    public partial class KapiScrewModel
    {
        ...
        public KapiScrewGiftBagConfig GetOptionalGiftActivityConfig()
        {
            return KapiScrewModel.Instance.GiftBagConfig[0];
        }
        ...
        public void PurchaseSuccess(TableShop tableShop)
        {
            ...
            if (config != null)
            {
                ...
                var controller= UIManager.Instance.GetOpenedUIByPath<UIPopupKapiScrewShopController>(UINameConst.UIPopupKapiScrewShop);
                if(controller!=null)
                    controller.AnimCloseWindow();
                if (ScrewGameLogic.Instance.context!=null && ScrewGameLogic.Instance.context.boostersView != null)
                {
                    ScrewGameLogic.Instance.context.boostersView.RefreshUI();
                }
            }
        }
        ...
        public List<ResData> GetGiftBagRewards()
        {
            List<ResData> resDatas = new List<ResData>();
            if (!IsInitFromServer())
                return resDatas;
            ...
            if (config != null)
            {
                ...
                else
                {
                    if (StorageOptionalGift.SelectItem.ContainsKey(0))
                    {
                        resDatas.Add(new ResData(StorageOptionalGift.SelectItem[0],
                            config.Count1[config.Item1.IndexOf(StorageOptionalGift.SelectItem[0])]));
                    }
                }
                ...
                else
                {
                    if (StorageOptionalGift.SelectItem.ContainsKey(1))
                    {
                        resDatas.Add(new ResData(StorageOptionalGift.SelectItem[1],
                            config.Count2[config.Item2.IndexOf(StorageOptionalGift.SelectItem[1])]));
                    }
                }
                ...
                else
                {
                    if (StorageOptionalGift.SelectItem.ContainsKey(2))
                    {
                        resDatas.Add(new ResData(StorageOptionalGift.SelectItem[2],
                            config.Count3[config.Item3.IndexOf(StorageOptionalGift.SelectItem[2])]));
                    }
                }
            }
