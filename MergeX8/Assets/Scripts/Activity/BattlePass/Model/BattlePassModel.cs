...
namespace Activity.BattlePass
{
    public partial class BattlePassModel : ActivityEntityBase
    {
        ...
        public override string Guid => "OPS_EVENT_TYPE_BATTLEPASS";
        ...
        {
            if (StorageManager.Instance.GetStorage<StorageHome>().BattlePass.ContainsKey(StorageKey))
            {
                storageBattlePass = StorageManager.Instance.GetStorage<StorageHome>().BattlePass[StorageKey];
            ...
            else
            {
                ...
                storageBattlePass.PayLevelGroup = PayLevelModel.Instance.GetCurPayLevelConfig().BattlePass1GroupId;
                storageBattlePass.LoopRewardConfigToJson = JsonConvert.SerializeObject(BattlePassConfigManager.Instance._battlePassLoopRewardConfigs);
                StorageManager.Instance.GetStorage<StorageHome>().BattlePass.Add(StorageKey, storageBattlePass);
            }
        ...
        public bool IsLocalPurchasing()
        {
            ...
            foreach (var kv in StorageManager.Instance.GetStorage<StorageHome>().BattlePass)
            {
        ...
        {
            DebugReset();
            StorageManager.Instance.GetStorage<StorageHome>().BattlePass.Clear();
        }
        ...
        {
            "prefabs/activity/battlepass.ab",
            "spriteatlas/activityatlas/battlepassatlas/hd.ab",
            "spriteatlas/activityatlas/battlepassatlas/sd.ab",
        };
        ...
        public static bool CheckActivityEnd()
        {
            ...
            {
                StorageManager.Instance.GetStorage<StorageHome>().BattlePass.Remove(key);
                DebugUtil.LogError("-----------bp1 活动到期且领取所有奖励 删除 " + key);
            }
            ...
            if (storageBattlePass != null && ResExist())
            {
                ...
                if (UnlockManager.IsOpen(UnlockManager.MergeUnlockType.BattlePass))
                {
                    UIManager.Instance.OpenUI(UINameConst.UIPopupBattlePassEnd, storageBattlePass);
                    return true;
                }
            }
        ...
        {
            int loopCount = 0;
            int maxLoopCount = StorageManager.Instance.GetStorage<StorageHome>().BattlePass.Count;
        ...
        {
            List<string> keys = StorageManager.Instance.GetStorage<StorageHome>().BattlePass.Keys.ToList();
            ...
            if (isActivityEndBuy)
            {
                for (int i = keys.Count - 1; i >= 0; i--)
                {
                    ...
                    StorageBattlePass storageBattlePass =
                        StorageManager.Instance.GetStorage<StorageHome>().BattlePass[key];
                    ...
                    if (IsActivityEnd(storageBattlePass))
                    {
                        ...
                        EventDispatcher.Instance.DispatchEventImmediately(EventEnum.BATTLE_PASS_PURCHASE, true);
                        break;
                    }
                }
            }
            ...
            {
                StorageBattlePass storageBattlePass =
                    StorageManager.Instance.GetStorage<StorageHome>().BattlePass[StorageKey];
                if(!storageBattlePass.IsPurchase)
        ...
        public bool CanShow()
        {
            ...
            string openView = UINameConst.UIBattlePassMain;
            if (!storageBattlePass.IsShowStart)
            {
                if (UIManager.Instance.GetOpenedUIByPath(UINameConst.UIPopupBattlePassStart))
                    return false;

                openView = UINameConst.UIPopupBattlePassStart;
            }
            else
            {
                return false;
                // if (UIManager.Instance.GetOpenedUIByPath(UINameConst.UIBattlePassMain))
                //     return false;
            }
        ...
        public bool CanShowRefresh()
        {
            ...
            if (Activity.BattlePass.BattlePassTaskModel.Instance.battlePassTask.RefreshTime == CommonUtils.GetTomorrowTimestamp((long)APIManager.Instance.GetServerTime()))
                return false;
            
            return Activity.BattlePass.UIPopupBattlePassRefreshController.CanShow();
        }
