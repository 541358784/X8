...
namespace Activity.BattlePass
{
    ...
    {
        private static Dictionary<StorageBattlePass, List<TableBattlePassLoopRewardConfig>> LoopRewardConfigDic = new Dictionary<StorageBattlePass, List<TableBattlePassLoopRewardConfig>>();
        public static List<TableBattlePassLoopRewardConfig> GetLoopRewardConfigs(this StorageBattlePass storage)
        {
            if (storage.LoopRewardConfigToJson.IsEmptyString())
                return null;
            if (!LoopRewardConfigDic.TryGetValue(storage, out var result))
            {
                result = JsonConvert.DeserializeObject<List<TableBattlePassLoopRewardConfig>>(storage.LoopRewardConfigToJson);
