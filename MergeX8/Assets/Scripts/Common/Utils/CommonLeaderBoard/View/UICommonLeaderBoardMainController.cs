...
public class UICommonLeaderBoardMainController:UIWindowController
{
    ...
    //     // Failed,
    // }
    public CommonLeaderBoardModel LeaderBoardModel;
    ...
    public Dictionary<int,RankNode> _rankNodeDictionary;
    public Transform _playerDefaultItem;
    ...
    public override void PrivateAwake()
    {
        ...
        LockText = transform.Find("Root/Locked/Image (1)/Text").GetComponent<LocalizeTextMeshProUGUI>();
    ...
    public void RefreshLockView()
    {
        ...
        LockText.SetTermFormats(_storageWeek.LeastStarCount.ToString());
    }

    protected override async void OnOpenWindow(params object[] objs)  
    {
        base.OnOpenWindow(objs);
        var storageWeek = objs[0] as StorageCommonLeaderBoard;
        LeaderBoardModel = objs[1] as CommonLeaderBoardModel;
        var iconSprite = GetCollectItemIcon(storageWeek);
        if (iconSprite != null)
        {
            foreach (var pair in _rankNodeDictionary)
            {
                pair.Value.transform.Find("Image (3)").GetComponent<Image>().sprite = iconSprite;
            }
            _playerDefaultItem.Find("Normal/BGGroup/BG/Icon").GetComponent<Image>().sprite = iconSprite;
            _playerDefaultItem.Find("My/BGGroup/BG/Icon").GetComponent<Image>().sprite = iconSprite;
        }
        BindStorageWeek(storageWeek);
        RefreshUI();
    }

    public virtual Sprite GetCollectItemIcon(StorageCommonLeaderBoard storageWeek)
    {
        if (storageWeek != null && storageWeek.CollectItemResourceId != 0)
        {
            var collectType = (UserData.ResourceId) storageWeek.CollectItemResourceId;
            var iconSprite = UserData.GetResourceIcon(collectType, UserData.ResourceSubType.Big);
            return iconSprite;
        }
        return null;
    }
    
    public void BindStorageWeek(StorageCommonLeaderBoard storageWeek)
    {
    ...
    public void RefreshUI()
    {
        ...
        _buttonClose.gameObject.SetActive(!_storageWeek.CanStorageCommonLeaderBoardGetReward());
        _buttonCollect.gameObject.SetActive(_storageWeek.CanStorageCommonLeaderBoardGetReward());
        _buttonShop.gameObject.SetActive(false);
        //更新ui排名逻辑
    ...
    async void OnCollectBtn()
    {
        ...
        if (_storageWeek.CanStorageCommonLeaderBoardGetReward())
        {
            var success = await _storageWeek.QuitLeaderBoardFromServer();
            if (!success)
                return;
            //没有BI暂时注释掉
            var reasonArgs = new GameBIManager.ItemChangeReasonArgs(){reason = LeaderBoardModel.GetItemChangeReason()};
            var sortController = _storageWeek.SortController();
            var unCollectRewards = LeaderBoardModel.GetRewardsByRank(_storageWeek,sortController.MyRank);
            _storageWeek.CompletedStorageActivity();
            DragonPlus.GameBIManager.Instance.SendGameEvent(
                LeaderBoardModel.LeaderBoardFinishGameEventType(),
    ...
    //         return;
    //     var startPopup = UIManager.Instance.OpenUI(UINameConst.UICommonLeaderBoardStart,true) as UICommonLeaderBoardStartController;
    // }
