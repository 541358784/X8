...
namespace TMatch
{
    public class UitMatchTimeOutViewParam : UIViewParam
    {
        public Action<bool> handleClick;
    }

    [AssetAddress("TMatch/Prefabs/UISettlementTimeout")]
    public class UITMatchTimeOutController : UIPopup
    {
        public override Action EmptyCloseAction => null;
        private UitMatchTimeOutViewParam _viewParam;
        ...
        {
            Revive revive = TMatchConfigManager.Instance.GetRevive(1, TMatchPruchaseBoostSystem.Instance.RetainPurchaseCntInSingleLevel);
            ...
            {
                // EventDispatcher.Instance.DispatchEvent(new TMatchBoostUseEvent(revive.rewardID[i], revive.rewardCnt[i]));
                ...
                if (item.GetItemType() == ItemType.TMClock)
                {
                    var totalTime = item.effectValue1 * revive.rewardCnt[i];
                    transform.Find($"Root/Root1/InsideGroup/PropGroup/Item1/Text").GetComponent<TextMeshProUGUI>().SetText($"+{totalTime}s");
                }
                else if (item.GetItemType() == ItemType.TMLighting)
                {
                    transform.Find($"Root/Root1/InsideGroup/PropGroup/Item2/Text").GetComponent<TextMeshProUGUI>().SetText($"X{revive.rewardCnt[i] * item.effectValue1}");
                }
                else
                {
                    transform.Find($"Root/Root1/InsideGroup/PropGroup/Item2/Text").GetComponent<TextMeshProUGUI>().SetText($"X{revive.rewardCnt[i]}");
                }
            }

            if (revive.rewardID.Length == 1)
            {
                transform.Find($"Root/Root1/InsideGroup/+Text").gameObject.SetActive(false);
                transform.Find($"Root/Root1/InsideGroup/PropGroup/Item2").gameObject.SetActive(false);
            }
        }

        private void InitAdUI()
        {
            // if (!AdLogicManager.Instance.ShouldShowRV(eAdReward.TMatchReviveTimeOut, true)) {
            //     transform.Find($"Root/Root2").gameObject.SetActive(false);
            // }else {
            //     Revive revive = TMatchConfigManager.Instance.GetRevive(1, TMatchPruchaseBoostSystem.Instance.RetainPurchaseCntInSingleLevel);
            //            
            //     for (int i = 0; i < revive.AdRewardID.Count; i++)
            //     {
            //         // EventDispatcher.Instance.DispatchEvent(new TMatchBoostUseEvent(revive.AdRewardID[i], revive.AdRewardCnt[i]));
            //         var rewardId = revive.AdRewardID[i];
            //         DragonPlus.Config.Game.Item item = TMatch.GameConfigManager.Instance.GetItem(rewardId);
            //         if (item.GetItemType() == ItemType.Clock) {
            //             var totalTime = (int)(item.effectValue1 * revive.AdRewardCnt[i]);
            //             transform.Find($"Root/Root2/ADButton/PropItem/Text").GetComponent<TextMeshProUGUI>().SetText($"+{totalTime}s");
            //         }
            //     }
            // }
            transform.Find($"Root/Root2").gameObject.SetActive(false);
        ...
        private void RefreshLoseItem()
        {
            ...
            transform.Find($"Root/Root1/InsideGroup1/Items1").gameObject.SetActive(loseItemDatas.Count > 4);
            ...
            var transItem = transform.Find($"Root/Root1/InsideGroup1/Items/Items");
            ...
            for (var i = 0; i < loseItemDatas.Count; i++)
            {
                ...
                if (i > 3)
                {
                    parent = transform.Find($"Root/Root1/InsideGroup1/Items1");
                }
        ...
        public override void OnViewOpen(UIViewParam data)
        {
            ...
            InitLoseUI();
            InitBottomShow();
            _viewParam = data as UitMatchTimeOutViewParam;
        ...
        private void OnBuy()
        {
            ...
            var revive =
                TMatchConfigManager.Instance.GetRevive(1,
        ...
        private async void ContinueOnClick()
        {
            ...
            Revive revive = TMatchConfigManager.Instance.GetRevive(1, TMatchPruchaseBoostSystem.Instance.RetainPurchaseCntInSingleLevel);
            // if (!CurrencyModel.Instance.CanAford(ResourceId.Coin, revive.reviveCost))
            ...
            if (!canFreeRevive)
            {
                ...
                {
                    reason = BiEventAdventureIslandMerge.Types.ItemChangeReason.TimeReliveTm
                });
            ...
            //     new DragonPlus.GameBIManager.ItemChangeReasonArgs(BiEventMatchFrenzy.Types.ItemChangeReason.TimeRelive));
        
            UIViewSystem.Instance.Close<UITMatchTimeOutController>();
            // GameBIManager.Instance.LevelInfo.TimeRespawnCount++;
            for (int i = 0; i < revive.rewardID.Length; i++)
            {
        ...
        {
            // if (AdLogicManager.Instance.ShouldShowRV(eAdReward.TMatchReviveTimeOut, false))
            // {
            //     AdLogicManager.Instance.TryShowRewardedVideo(eAdReward.TMatchReviveTimeOut, (success, str) => {
            //         if (success) {
            //             Revive revive = TMatchConfigManager.Instance.GetRevive(1, TMatchPruchaseBoostSystem.Instance.RetainPurchaseCntInSingleLevel);
            //             TMatchPruchaseBoostSystem.Instance.RetainPurchaseCntInSingleLevel++;
            //             UIViewSystem.Instance.Close<UITMatchTimeOutController>();
            //             GameBIManager.Instance.LevelInfo.TimeRespawnAdCount++;
            //             for (int i = 0; i < revive.AdRewardID.Count; i++)
            //             {
