...
namespace DataInspector
{
    public static class GUIContainerTools
    {
        ...
        private class DictionaryGUIState
        {
            ...
            public int Size()
            {
                return parser.Size(dict.Target);
            }
            ...
            {
                return parser.Get(dict.Target, key);
            }

            public void Set(object key, object value)
            {
                parser.Set(dict.Target, key, value);
            }

            public Type ValueType()
            {
                return parser.ValueType(dict.Target);
            }

            public bool Resizeable()
            {
                return parser.Resizable(dict.Target);
            }

            public object Resize(int size)
            {
                return parser.Resize(dict.Target, size);
            }
        }

        private static readonly Dictionary<string, DictionaryGUIState> guiCache =
            new Dictionary<string, DictionaryGUIState>();
        ...
        private static bool Traversal(Inspector inspector, string path, DictionaryGUIState state, int start, int end)
        {
            ...
            else
            {
                ...
                for (int inner = start; inner < end; inner += step)
                {
                    ...
                    inspector.isFoldout[foldoutPath] = GUITools.Foldout(
                        inspector.isFoldout.ContainsKey(foldoutPath) && inspector.isFoldout[foldoutPath], label);
        ...
        private static bool InspectElement(Inspector inspector, string path, object key, DictionaryGUIState state,
            Type dictValueType)
        {
