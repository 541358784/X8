...
namespace DataInspector
{
    public partial class Inspector
    {
        ...
        public class Options
        {
            ...
            public int
                listBucketSize =
                    30; // If container (list or dictionary) is too large, the elements would not be shown straight forward
        ...
        ///////////////////////////////////////////////////////////////
        // Extra init step
        private static readonly Dictionary<string, Action<Inspector>> OnRegisterDefaultVisualizers =
            new Dictionary<string, Action<Inspector>>();
                ...
                if (visualizer.HasChildren())
                {
                    ...
                    if (!alwaysShowChildren)
                    {
                        using (GUITools.HorizontalScope())
                        {
                            ...
                            {
                                isFoldout[path] = GUITools.Foldout(isFoldout.ContainsKey(path) && isFoldout[path],
                                    fieldinfo);
                            }
