...
public class UIDailyPack2Controller : UIWindowController
{
    ...
    private List<UIDailyBundleItem2> _bundleItems;
    public override void PrivateAwake()
    {
        _bundleItems = new List<UIDailyBundleItem2>();
    ...
    protected override void OnOpenWindow(params object[] objs)
    {
        ...
        for (int i = 0; i < packInfo.Count-1; i++)
        {
            var packItem = Instantiate(rewardItems,rewardParent).gameObject.AddComponent<UIDailyBundleItem2>();
        ...
        var rewardItems2 = rewardParent.Find("RewardItem2");
        rewardItems2.gameObject.SetActive(false);
        {
            var lastPackInfo = packInfo[packInfo.Count - 1];
            var packItem = Instantiate(rewardItems2,rewardParent).gameObject.AddComponent<UIDailyBundleItem2>();
            packItem.transform.localScale=Vector3.one;
            packItem.gameObject.SetActive(true);
            var pi = AdConfigHandle.Instance.GetDailyPackInfoById(lastPackInfo.PackInfoId);
            packItem.Init(lastPackInfo, pi, source);
            _bundleItems.Add(packItem);
        }
    ...
    {
        if (!DailyPackModel.Instance.IsOpen())
            return false;
