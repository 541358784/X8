...
public class Aux_BattlePass : Aux_ItemBase
{
    ...
    {
        bool isOpen = Activity.BattlePass.BattlePassModel.Instance.IsOpened();
        gameObject.SetActive(isOpen);
        if (gameObject.activeSelf)
        {
            _timeText.SetText(Activity.BattlePass.BattlePassModel.Instance.GetActivityLeftTimeString());
        }
    ...
    {
        if (!Activity.BattlePass.BattlePassModel.Instance.storageBattlePass.IsShowStart)
        {
            GameBIManager.Instance.SendGameEvent(BiEventAdventureIslandMerge.Types.GameEventType.GameEventEasterPop);
            UIManager.Instance.OpenUI(UINameConst.UIPopupBattlePassStart);
        }
        else
        {
