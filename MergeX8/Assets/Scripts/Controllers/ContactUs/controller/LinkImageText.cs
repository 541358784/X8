...
public class LinkImageText : Text, IPointerClickHandler
{
    ...
    protected override void OnPopulateMesh(VertexHelper toFill)
    {
        ...
        foreach (var hrefInfo in m_HrefInfos)
        {
            ...
            pos = new Vector3(pos.x, pos.y - addHeight, pos.z);
            ...
            for (int i = hrefInfo.startIndex, m = hrefInfo.endIndex; i < m; i++)
            {
                ...
                {
                    Vector3 a = bounds.size;
                    var rt1 = new Rect(bounds.min,
                        new Vector3(bounds.size.x, bounds.size.y + addHeight * 2, bounds.size.z));
                    hrefInfo.boxes.Add(rt1);
            ...
            var rt2 = new Rect(bounds.min, new Vector3(bounds.size.x, bounds.size.y + addHeight * 2, bounds.size.z));
            hrefInfo.boxes.Add(rt2);
        }
    }
    ...
    protected virtual string GetOutputText(string outputText)
    {
        ...
        foreach (Match match in s_HrefRegex.Matches(outputText))
        {
            ...
            s_TextBuilder.Append("<color=#3ca74b>"); // 超链接颜色
            var baseIndex = 15;

            int whiteSpaceNum = 0;
            for (int charStart = 0; charStart < s_TextBuilder.Length; charStart++)
            {
                if (Char.IsWhiteSpace(s_TextBuilder.ToString(), charStart))
                {
                    whiteSpaceNum++;
                }
            }
            ...
            {
                startIndex = (s_TextBuilder.Length - whiteSpaceNum - baseIndex) * 4, // 超链接里的文本起始顶点索引
                endIndex = (s_TextBuilder.Length + match.Groups[2].Length - 1 - whiteSpaceNum) * 4 + 3,
                name = group.Value
            };
