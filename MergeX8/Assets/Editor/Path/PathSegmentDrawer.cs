    ...
    public class PathSegmentDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            using (new EditorGUI.PropertyScope(rect, label, property))
            {
                ...
                {
                    width = rect.width / count - 10,
                };

                var point2Rect = new Rect(point1Rect)
                {
                    x = rect.width / count * index,
                };
                index++;
                ...
                beginProperty.objectReferenceValue = EditorGUI.ObjectField(point1Rect, beginProperty.objectReferenceValue, typeof(Transform), true);
                endProperty.objectReferenceValue = EditorGUI.ObjectField(point2Rect, endProperty.objectReferenceValue, typeof(Transform), true);
