...
namespace DragonU3DSDK
{
    public class EditorResourcePaths
    {
        ...
        public const string GROUP_CONFIG_NAME = "_resourceGroupConfig.json";
        public const string GROUP_CONFIG_PATH = ASSETS_ROOT + GROUP_CONFIG_NAME;
        public const string DEFAULT_GROUP_CONFIG_PATH = ASSETS_ROOT + "DragonSDK/DragonU3DSDK/Framework/Editor/" + GROUP_CONFIG_NAME;
        public const string META_SUFFIX = ".meta";
        public const string ATLAS_SUFFIX = ".spriteatlas";
        public const string FILTER_META_REGEX = "^(?!.*\\.((meta)|(DS_Store))$)";
        public const string PNG_REGEX = ".*\\.((png)|(jpg))$";
        public const string FILTER_PEM_REGEX = ".*\\.((pem))$";
        ...
        static List<PathPattern> ReadConfig(string key)
        {
            ...
            {
                EditorCommonUtils.CopyFile(DEFAULT_CONFIG_PATH, CONFIG_PATH);
        ...
        public static List<List<string>> ReadGroupConfig()
        {
            if (!File.Exists(GROUP_CONFIG_PATH))
            {
                File.Copy(DEFAULT_GROUP_CONFIG_PATH, GROUP_CONFIG_PATH);
                AssetDatabase.Refresh();
            }

            string text = File.ReadAllText(GROUP_CONFIG_PATH);
            var groups = JsonConvert.DeserializeObject<Dictionary<string, List<List<string>>>>(text)["ABRes_Group"];
