...
namespace DragonU3DSDK
{
    public class EditorStabilityTool
    {
        ...
        [MenuItem("Assets/稳定性/核实Manager不是通过手动挂载的")]
        static void CheckManagerNotByHandle()
        {
            ...
            foreach (var p in prefabs)
            {
                ...
                Component[] coms = obj.GetComponentsInChildren<Component>(true);
                foreach (var a in coms)
                {
                    if(a == null)
                        continue;
                    
                    Type t = a.GetType().BaseType;
                    if (a.GetType().Name.IndexOf("AsyncTaskManager") == 0)
                    {
                        str += $"{a.GetType()}" + " ";
                    }
                    if (t != null && (t.Name.IndexOf("Manager") == 0))
                    {
                        str += $"{a.GetType()}" + " ";
                    }
                }
                ...
                {
                    DebugUtil.LogError($"prefab : {path} {str}");
                }
            }
            
            DebugUtil.Log("核实Manager不是通过手动挂载的 完成");
        }
    }
}