...
namespace DragonU3DSDK.Config
{
    public class TableManager : Manager<TableManager>
    {
        ...
        public List<T> GetTable<T>(string tableLocation) 
        {
            TextAsset json = GetTextAsset(typeof(T),tableLocation);
            if(json == null){
                //Fixed NUll reference, 如果没有找到本地的version文件，需要拷贝，return true ，否则会报错卡死进度条
                return new List<T>();
            }
        ...
        public TextAsset GetTextAsset(System.Type type,string tableLocation)
        {
            string className = type.ToString().ToLower().Replace("table", "");
            if(Debug.isDebugBuild && Directory.Exists(tableLocation))
            {//测试修改table的功能
                TextAsset json = new TextAsset(File.ReadAllText(tableLocation + "/" + className + ".json"));
                return json;
            }
            else
            {//使用正常配置
                TextAsset json = Asset.ResourcesManager.Instance.LoadResource<TextAsset>(Path.Combine(tableLocation,className));
                return json;
            }
        }
