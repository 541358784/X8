...
namespace DragonU3DSDK
{
    public class SystemInfomation : Manager<SystemInfomation>
    {
        ...
        static void RequestADIDCallback(string adid, bool success, string error)
        {
            if (success)
            {
                DebugUtil.Log(" SystemInfomation completed , and trigger the event, adid = {0}", adid);
                ADID = adid;
#if DEBUG
                if (!string.IsNullOrEmpty(ADID))
                {
                    Dlugin.SDK.GetInstance().m_AdsManager?.SetTestDeviceAdvertisingIdentifiers(ADID);
                }
#endif
            }
        }
