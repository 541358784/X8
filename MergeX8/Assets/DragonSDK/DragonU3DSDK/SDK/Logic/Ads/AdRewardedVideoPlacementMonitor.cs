/*
 * 可以监控的状态：
 * Active【care parent】、Interactable、InsideScree、ShouldShowRV、Custom
 */
...
{
    public class AdRewardedVideoPlacementMonitor : MonoBehaviour, AdPlacementMonitor
    {
        private static Camera camera;
        public static void BindUICamera(Camera _camera)
        {
            camera = _camera;
        }
        ...
        {
            if (obj.GetComponent<AdRewardedVideoPlacementMonitor>() != null)
            {
                DebugUtil.LogError("$已经绑定了");
                return;
            }
            
            obj.AddComponent<AdRewardedVideoPlacementMonitor>().Bind(_placementId, _customCheck);
        }
        ...
        public void Check()
        {
            if (Time.realtimeSinceStartup - lastCheckTime > 0.1f)
            {
                ...
                if (string.IsNullOrEmpty(placementId) || GetComponent<Button>() == null || camera == null)
                {
                    return;
                }
                ...
                if (!validShould)
                {
                    if (Interactable() &&
                        ...
                        InsideScree() &&
                        AdLogicManager.Instance.ShouldShowRV(placementId, false) &&
                        Custom())
                    {
                        validShould = true;
                    
                        DragonU3DSDK.Network.BI.BIManager.Instance.SendCommonMonetizationAdEvent(
                            BiEventCommon.Types.CommonMonetizationAdEventType.CommonMonetizationEventRewardVideoShouldDisplay, placementId, 
                        ...
                        DebugUtil.Log($"曝光检测 激励广告位：{placementId} 曝光一次");
                    }
                }
                else
                {
