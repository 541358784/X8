...
extern "C"
{
   ...
   void _MaxLoadMRec(const char *adUnitIdentifier)
   {
    ...
    void _MaxDisposeInterstitialAd(const char* adUnitIdentifier)
    {
        if ( !_initializeSdkCalled )
        {
            max_unity_log_error(@"Dispose Interstitial ad must after MAX SDK is initialized");
            return;
        }
        
        [getAdManager() disposeInterstitialAd: NSSTRING(adUnitIdentifier)];
    }
    
    void _MaxDisposeRewardedVideoAd(const char* adUnitIdentifier)
    {
        if ( !_initializeSdkCalled )
        {
            max_unity_log_error(@"Dispose RewardedVideo ad must after MAX SDK is initialized");
            return;
        }
        
        [getAdManager() disposeRewardedVideoAd: NSSTRING(adUnitIdentifier)];
    }
