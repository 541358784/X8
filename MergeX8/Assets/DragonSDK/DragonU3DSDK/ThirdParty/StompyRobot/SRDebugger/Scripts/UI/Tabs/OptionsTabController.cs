...
namespace SRDebugger.UI.Tabs
{
    ...
    public class OptionsTabController : SRMonoBehaviourEx
    {
        ...
        [RequiredField] public Button SeatchBtn;
        
        [RequiredField] public InputField SearchInput;
        ...
        protected override void Start()
        {
            ...
            SeatchBtn.onClick.AddListener(() =>
            {
                if (SearchInput.text.IsEmptyString())
                {
                    foreach (var cat in _categories)
                    {
                        cat.CategoryGroup.gameObject.SetActive(true);
                    }
                }
                else
                {
                    foreach (var cat in _categories)
                    {
                        cat.CategoryGroup.gameObject.SetActive(cat.CategoryGroup.Header.text.ToLower().Contains(SearchInput.text.ToLower()));
                    }
                }
            });
        }
