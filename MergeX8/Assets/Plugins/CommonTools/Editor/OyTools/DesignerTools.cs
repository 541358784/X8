...
using UnityEngine;
public class #Name#Controller : #Name# {
}
";
    const string view_temp_str = @"
using UnityEngine;
using UnityEngine.UI;
public abstract class #Name# : MonoBehaviour {
#Properties#
    public void Awake(){
#GetProperties#
...
/// #FULL_PATH#
/// </summary>
public abstract class #Name# : UIWindow {
...
public class #Name#Controller : #Name# {
    ...
    {
        //#Name#Controller window = UIManager.Instance.OpenUI(AssetPath, objs) as #Name#Controller;
...
";
   ...
   static void SetFullRect( RectTransform t) {
    ...
    private static void ExportScript (bool isWindow, GameObject selectObj) {
        ...
        //     findChild(trans);
        // }
        string exportType = selectTrans.gameObject.tag;
        // UnityEngine.Debug.LogError ("tag=" + exportType);
        if (exportType == "Export") { //按最少对象导出，只导出标记需要（Export）导出的，减少生成太多无用的对象。
        ...
        basecontent = basecontent.Replace ("#AssetPath4UIWindow#", $"\"{assetPath4UIWindow}\"");
        basecontent = basecontent.Replace ("#AssetPath4Mono#", $"\"{assetPath4MonoBehaviour}\"");
        basecontent = basecontent.Replace ("#Name#", name); //Path.GetFileNameWithoutExtension (scriptPath));
        basecontent = basecontent.Replace ("#Properties#", propertiesStr.ToString ());
        basecontent = basecontent.Replace ("#GetProperties#", getPropertiesStr.ToString ());
        basecontent = basecontent.Replace ("#AddPropertiesListerner#", addListenerStr.ToString ());
        basecontent = basecontent.Replace ("#ClickListernerImpl#", clickListenStr.ToString ());
        basecontent = basecontent.Replace ("#ClickListernerOverride#", clickListenOverrideStr.ToString ());
        basecontent = basecontent.Replace ("#RemovePropertiesListerner#", removeListenerstr.ToString ());
    ...
    private static void findChild (Transform trans, Transform selectTrans, bool export) {
        ...
        */
        ...
        if (comp != null) {
            ...
            propertiesStr.AppendFormat ($"   protected {className} {menType}_{propName};\n");
            //获取组件在预制体中的路径
            ...
            getPropertiesStr.AppendFormat ($"        {menType}_{propName} = transform.Find(\"{compPath}\").GetComponent<{className}>();\n");
            if (comp.GetType () == typeof (Button)) {
                addListenerStr.AppendFormat ($"        {menType}_{propName}.onClick.AddListener({propName}Clicked);\n");
                clickListenStr.AppendFormat ($"    public abstract void {propName}Clicked();\n");
                clickListenOverrideStr.AppendFormat ("    public override void {0}Clicked(){1}\n", propName, "{}");
                removeListenerstr.AppendFormat ($"        {menType}_{propName}.onClick.RemoveAllListeners();\n");
            }
        } else {
            if (!export && !trans.CompareTag("Export")) {
            ...
            getPropertiesStr.AppendFormat ($"        {menType}_{propName} = transform.Find(\"{compPath}\") as {className};\n");
        }
